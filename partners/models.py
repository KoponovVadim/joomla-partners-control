...
class DonorSite(TimeStampedModel):
    ...
    joomla_version = models.CharField("Версия Joomla", max_length=10, choices=JoomlaVersion.choices, default=JoomlaVersion.UNKNOWN)
    ...
    template = models.ForeignKey(PageTemplate, verbose_name="Шаблон", null=True, blank=True, on_delete=models.SET_NULL)
    enabled = models.BooleanField("Активен", default=True)
    connection_status = models.CharField(max_length=30, choices=ConnectionStatus.choices, default=ConnectionStatus.UNKNOWN)
    ...
    class Meta:
        ordering = ["domain"]
    def __str__(self): return self.name
    def get_absolute_url(self): return reverse("donor-edit", args=[self.pk])
    @property
    def password_is_set(self): return bool(self.encrypted_password)
...
class ClientSite(TimeStampedModel):
    ...
    class Meta:
        ordering = ["name"]
    def __str__(self): return self.name
    def get_absolute_url(self): return reverse("client-edit", args=[self.pk])
...
class Placement(TimeStampedModel):
    ...
    def __str__(self): return f"{self.client} → {self.donor}"
